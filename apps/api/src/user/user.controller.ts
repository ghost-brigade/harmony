import { Controller, Post, Body, Patch, Get, Delete, Param, UseGuards, HttpStatus, Res, HttpException } from "@nestjs/common";
import { Throttle } from "@nestjs/throttler";
import { UserService } from "./user.service";
import { ZodGuard } from "../core/guards/zod/zod.guard";
import { createUserSchema, createUserType, publicUserSchema, publicUserType, updateUserSchema, updateUserType, userSchema } from "@harmony/zod";
...
export class UserController {
  ...
  async findAll(): Promise<publicUserType[]> {
    try {
      ...
      return users.map(user => publicUserSchema.parse(user)) as publicUserType[];
    } catch (error) {
      console.log(error);
    }
  }
